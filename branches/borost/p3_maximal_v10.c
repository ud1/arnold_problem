/*
 * p3_maximal_v10.c — Generate p3-maximal pseudoline arrangements.
 *
 * Algorithm: Bokowski-Roudneff-Strempel, Section 4.
 * Improvements over v9:
 *   - Bidirectional extension (left + right), as in the original paper
 *   - Block-endpoint filtering: skip interior elements of adjacency chains
 *
 * Compile: gcc -O2 -o p3v10c tools/p3_maximal_v10.c
 * Usage:   ./p3v10c <n> [max_results]
 */
...
static int chi_trail[N_MAX*N_MAX*N_MAX*6]; /* stores triples: idx*3+{0,1,2} */
...
static int adj_trail[N_MAX*N_MAX*N_MAX*4]; /* pairs: idx*2+{0,1} */
...
static int right_arr[N_MAX+1][N_MAX]; /* right_arr[p][0]=last, [1]=first right-placed, ... */
static int right_len[N_MAX+1];
static uint32_t rem_mask[N_MAX+1];    /* gap between left and right frontiers */
...
/* An element is a block endpoint on hyperline p if it has < 2
   neighbors among remaining elements (rem_mask). Interior elements
   (2 remaining-neighbors) cannot be placed next from either side. */

static inline int is_block_endpoint(int p, int e) {
    int nc = nbr_count[p][e];
    if (nc < 2) return 1; /* can't have 2 remaining-neighbors */
...
static int get_left_candidates(int p, int *cands) {
    ...
    while (mask) {
        ...
        /* Block-endpoint filter: skip interior elements */
        ...
        /* Chi ordering check: no remaining predecessor
           (no x in rem that must come before e on the left side) */
        int se = sp[e];
        int ok = 1;
...
static int get_right_candidates(int p, int *cands) {
    int8_t *sp = sgn[p];
    uint32_t rp = rem_mask[p];
    int f = right_arr[p][right_len[p] - 1]; /* right frontier */
    int cnt = 0;
    ...
    while (mask) {
        ...
        /* Block-endpoint filter */
        if (!is_block_endpoint(p, e))
            continue;

        /* Pre-filter: triangle (p,e,f) feasible? */
        if (!can_add_adj(p, e, f) || !can_add_adj(e, p, f) || !can_add_adj(f, p, e))
            continue;

        /* Chi ordering check: no remaining successor
           (no x in rem that must come after e — all remaining should be before e) */
        int se = sp[e];
        int ok = 1;
        uint32_t mask2 = rp & ~(1u << e);
        while (mask2) {
...
static int place_left(int p, int e) {
    ...
    /* Gap closed? left frontier meets right frontier */
    if (rem_mask[p] == 0) {
...
static int place_right(int p, int e) {
    int8_t *sp = sgn[p];
    int se = sp[e];
    int f = right_arr[p][right_len[p] - 1]; /* old right frontier */
    ...
    /* Triangle (p, e, f) — e adjacent to old right frontier */
...
/* Build full sequence for hyperline p: left + reversed(right) */
...
static void solve(void) {
    ...
    /* Unit propagation: forced moves from either side */
    while (!contradiction) {
        int found_forced = 0;
        for (int p = 1; p < n; p++) {
            ...
            /* Force if only 1 candidate total, OR if only 1 on a specific side
               (a single-side forced move is also unambiguous) */
    ...
    /* Find branching hyperline.
       Canonical form: branch on LEFT candidates only.
       If left has 0 candidates but right has some, use right.
       This avoids duplicate search paths. */
    ...
    if (!contradiction) {
        ...
        for (int p = 1; p < n; p++) {
            ...
            /* Prefer left candidates; use right only if left is empty */
            int nc, sd;
            int *src;
...
int main(int argc, char **argv) {
    if (argc < 2) {
        fprintf(stderr, "Usage: p3_maximal_v10c <n> [max_results]\n");
        return 1;
    }
    int n = atoi(argv[1]);
